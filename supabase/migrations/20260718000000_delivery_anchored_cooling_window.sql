-- Elite Card Loyalty — delivery-anchored points cooling window
--
-- Anti-gaming rule: splitting one order into two bills a day apart must not
-- unlock points on the "2nd purchase". Points now start only N days (default
-- 30, admin-tunable via card_settings.points_cooling_days) after the
-- customer's first post-card purchase is DISPATCHED AND ASSEMBLED — proven by
-- a field-agent completion report in the system: a service_jobs row of type
-- delivery/self_delivery, status completed, with uploaded photos.
--
-- No field-verified completion report → the clock never starts → no points.
-- Builds on the consolidated points engine in 20260717011520.

-- ============================================================
-- 1. SETTING: cooling window length in days
-- ============================================================
...
-- ============================================================
-- 2. HELPER: when did the customer's first post-card purchase
--    get delivered & assembled (field-verified, with photos)?
--    Returns NULL when no verified completion exists yet.
-- ============================================================
...
-- ============================================================
-- 3. POINTS ENGINE: earn only when the bill falls on/after
--    (first verified delivery completion + cooling window)
-- ============================================================
...
BEGIN
  ...
  IF v_customer.card_tier IN ('super_elite','prestige_elite')
    ...
    -- Cooling window: anchored on the field-verified delivery + assembly
    -- completion (photos required) of the first post-card purchase.
